...
Date: January 9, 2024
Version: 1.0
Description: fixed expenditure component jsx for capy finance client
 */
...
export default function Fixed(props) {
    ...
    async function submitFixed(e) {
        ...
        const newFixed = { title, content, value, startDate, endDate, fExpId }
        const res = await axios.post(`${DOMAIN}/api/v1/fixed/${fExpId}`, newFixed)
        if (res?.data.success) {
    ...
    return (
        <div>
            ...
            {editMode && <form onSubmit={submitFixed} className="flex flex-col">
