...
Date: January 9, 2024
Version: 1.0
Description: liability component jsx for capy finance client
 */
...
export default function Liability(props) {
    ...
    async function submitLiability(e) {
        e.preventDefault()
        const liabilityId = props.liabilityId;
        ...
        const newAsset = { title, content, value, startDate, endDate, liabilityId }
        const res = await axios.post(`${DOMAIN}/api/v1/liabilities/${liabilityId}`, newAsset)
        if (res?.data.success) {
    ...
    return (
        <div>
            ...
            {editMode && <form onSubmit={submitLiability} className="flex flex-col">
